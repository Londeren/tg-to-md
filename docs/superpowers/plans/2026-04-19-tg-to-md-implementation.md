...
- `finished(out)` из `node:stream/promises` — одним вызовом ждём flush буфера и корректно реджектит promise при ошибке записи (EACCES, ENOSPC и т.п.). Ошибка поднимется в верхний `.catch()` → stderr + exit 1. Отдельная проверка `fs.existsSync` для input не нужна: `parseTelegramExport` откроет файл, получит ENOENT, и тот же catch выведет сообщение.
